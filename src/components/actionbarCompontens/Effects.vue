...
import catEffects from '@/assets/images/Effects/catEffects.png'
...
const applyFilter = (filterName) => {
  state.effectsData = state.effectsData === filterName ? 'notEffects' : filterName
}
</script>

<template>
  <div class="grid grid-cols-2 gap-4">
    <div v-for="(item, index) in arr" :key="index" @click="applyFilter(item.name)" class="cursor-pointer ">
      <div class="hover:border-black hover:border-[3px] bg-cover bg-no-repeat bg-size h-28 shadow-md rounded-xl"
        :style="`background-image:url(${item.image})`">
      </div>
      <p class="text-center font-[dindin]">{{ item.title }}</p>
    </div>
  </div>
</template>
